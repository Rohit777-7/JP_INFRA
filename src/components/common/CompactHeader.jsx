...
function CompactHeader({ eyebrow, title, description }) {
  ...
  return (
    <div ref={scope} className="shrink-0 border-b border-white/10 bg-navy-950 px-6 pt-20 pb-4 md:px-16">
      {eyebrow && (
        <p data-header-in className="mb-1 text-xs font-semibold tracking-[0.3em] text-brand-red uppercase">
          {eyebrow}
        </p>
      )}
      <div className="flex flex-wrap items-baseline justify-between gap-x-6 gap-y-1">
        <h1 data-header-in className="text-3xl text-white md:text-4xl">
          {title}
        </h1>
        {description && (
          <p data-header-in className="max-w-xl text-sm text-white/60">
            {description}
          </p>
        )}
      </div>
