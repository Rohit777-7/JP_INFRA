            ...
            <div className="flex justify-center gap-6 [perspective:1200px]">
              <div className="w-40 -rotate-6 bg-navy-700 p-5 text-white shadow-2xl transition-transform duration-500 hover:rotate-0 sm:w-48">
                <div className="flex justify-end">
                  <span className="border border-brand-red px-2 py-0.5 text-[9px] tracking-widest">JP INFRA</span>
                </div>
                <p className="mt-12 font-display text-2xl leading-none sm:text-3xl">{BRAND.tagline}</p>
                <p className="mt-2 text-[10px] text-white/70">{BRAND.subline}</p>
                <p className="mt-16 text-[9px] leading-relaxed text-white/50 sm:mt-20">{BRAND.rera}</p>
              </div>
              ...
              <div className="mt-8 w-40 rotate-6 bg-white p-5 text-navy-900 shadow-2xl transition-transform duration-500 hover:rotate-0 sm:w-48">
                <span className="border border-navy-900 px-2 py-0.5 text-[9px] tracking-widest">JP INFRA</span>
                <p className="mt-16 text-[10px] font-semibold text-navy-900 sm:mt-20">{BRAND.address}</p>
                <p className="mt-3 text-[10px] text-navy-900/70">{BRAND.email}</p>
                <p className="text-[10px] text-navy-900/70">{BRAND.website}</p>
                <p className="mt-3 font-display text-lg text-brand-red">{BRAND.tollFree}</p>
              </div>
            </div>

            <div>
              <h2 className="text-2xl text-navy-900 md:text-3xl">What's Inside</h2>
              ...
              {!gate ? (
                <form onSubmit={handleUnlock} className="mt-6 flex max-w-md flex-col gap-3 sm:flex-row">
                  <input
                    required
                    type="tel"
                    value={phone}
              ...
              <p className="mt-3 text-xs text-navy-900/40">{BRAND.disclaimer}</p>
            </div>
          </div>
        </div>
